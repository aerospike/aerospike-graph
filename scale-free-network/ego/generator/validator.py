...
def validate_aerospike_properties(properties):
    ...
    for prop_name, props in (properties or {}).items():
        ...
        if ptype.lower() not in AEROSPIKE_GRAPH_TYPES:
            raise ValueError(
                f"Invalid property type '{ptype}' for property '{name}'. "
                f"Supported types are: {sorted(AEROSPIKE_GRAPH_TYPES)}"
            )
        props["generator"] = src


def parse_connections_config(config, full_config, parent) -> Dict:
...
def parse_node_config(props, full_config) -> (Dict, Dict):
    """Validates connections properties are valid, and the connection has the distribution properties. Returns properties dict"""
    ...
    if not isinstance(properties, dict):
        raise ValueError(f"Properties are not a dict: '{properties}'")
    validate_aerospike_properties(properties)
    if not label or not isinstance(label, str):
        raise ValueError(f"Invalid Label Value: '{label}'")
    if connections:
        eprops = parse_connections_config(connections, full_config, label)

    return vprops, eprops


def parse_config(config) -> (Dict, Dict, Dict):
    ...
    for name, props in config.items():
        lname = name.lower()
        if lname == "egonode":
            vprop, eprops = parse_node_config(props, config)
            vert_props["EgoNode"] = vprop
            edge_props.update(eprops)
        else:
            for alter, alterProps in props.items():
                vprop, eprops = parse_node_config(alterProps, config)
