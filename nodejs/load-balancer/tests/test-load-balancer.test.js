...
async function setupGraph(rr) {
    ...
    await g.addE('owns').from_(user1.value).to(account1.value).next();
    await g.addE('owns').from_(user2.value).to(account2.value).next();
    await g.addE('owns').from_(user3.value).to(account3.value).next();
}
