...
class NationalID(BaseModel):
    ...
    def generate(cls, customer, address, fake, region):
        issued_date = fake.date_time_between(start_date="-30y", end_date="-5y").timestamp() * 1000
        date_of_birth = fake.date_time_between(start_date="-80y", end_date="-18y").timestamp() * 1000
        return cls(
            national_id=str(national_id_formats[region](fake)),
            issued_ctry=region_country_map[region],
            issued_date=int(issued_date),
